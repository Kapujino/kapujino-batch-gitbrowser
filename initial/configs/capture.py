...
def quickLaunch(titel='none'):
    ...
    if path.lower().startswith('activatewindow') and ',' in path: #i.e. NOT like ActivateWindow(filemanager)
        ...
        #label = xbmc.getInfoLabel('ListItem.Label')
               # titel='%s' % sys.listitem.getLabel()
                utils.log('path old %s' % str(path))
                #create new path
                searchterm = path.find('&site=globalSearch')
                new_path = path[:searchterm] + titel + path[searchterm:]
                xbmc.executebuiltin(new_path)
                return
        else:
